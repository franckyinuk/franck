...
function PlayerDataToString(p)
{
  return p.initials + ", " + p.total_points + ", " + p.num_victories + ", " + p.num_games + ", " + p.num_losses;
}
...
function GetSortingString(p, index)
{
  str = (10000+p.total_points)
    + ", " + (10000+p.num_games)
    + ", " + (10000+p.num_victories) 
    + ", " + (10000+p.num_losses)
    + ", " + (9999-index); // trick to get the sorting as given in the initial order
  Logger.log("GetSortingString: " + str);
  Logger.log(PlayerDataToString(p));
  return str;
}
...
function sort_assoc(aInput)
{
  ...
  {
    aTemp.push([GetSortingString(aInput[sKey], index), aInput[sKey]]);
    index++;
  }
  
  Logger.log("sort_assoc(" + aInput + ")");
  aTemp.sort();
  var aOutput = [];
...
function AreScoresValid(players_region, score_table_region)
{
  var ss = SpreadsheetApp.getActiveSpreadsheet();
...
function RefreshLeagueTable(initials, score_table, league_table_title, league_table)
{
  ...
  if (AreScoresValid(initials, score_table))
...
function RefreshLeagueTables()
{
  ...
  RefreshLeagueTable("initials_3", "scores_3", "league_table_title_3", "league_table_entries_3");
}
